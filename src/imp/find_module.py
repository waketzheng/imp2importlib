...
def find_module(name: str, path: "list[str] | None" = None) -> "tuple[IO | None, str, tuple[str, str, int]]":
    """
    ...
    if spec.submodule_search_locations is not None:
        if pathname.endswith("__init__.py"):
            try:
                file_obj = open(pathname, encoding="utf-8")
    ...
    if pathname.endswith(".py"):
        ...
        try:
            rb_file_obj = open(pathname, "rb")
            return (rb_file_obj, pathname, (".pyc", "rb", PY_COMPILED))
        except OSError:
